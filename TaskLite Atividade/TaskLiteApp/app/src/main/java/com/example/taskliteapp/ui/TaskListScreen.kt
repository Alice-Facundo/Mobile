...
import androidx.compose.foundation.layout.fillMaxWidth // Import for fillMaxWidth
...
@Composable
fun DesafioTaskListScreen(viewModel: DesafioTaskViewModel, modifier: Modifier = Modifier) {
    ...
    Column(modifier = modifier.fillMaxSize().padding(16.dp)) {
        ...
        // Filter Buttons Row
        Row(modifier = Modifier.fillMaxWidth()) { // <--- Added fillMaxWidth here
        ...
        Spacer(Modifier.height(16.dp)) // Spacer between filter buttons and task list

        // LazyColumn for Tasks
        LazyColumn(modifier = Modifier.weight(1f)) { // <--- Added weight to fill space
            items(taskList) { task ->
                TaskItemDesafio(
                    task = task,
        ...
        Spacer(Modifier.height(16.dp)) // Spacer between task list and input row
        ...
        Row(modifier = Modifier.fillMaxWidth()) { // <--- Added fillMaxWidth here
            ...
            Spacer(Modifier.width(8.dp))
            Button(onClick = {
                if (newTaskTitle.isNotBlank()) { // Added check for non-blank task
